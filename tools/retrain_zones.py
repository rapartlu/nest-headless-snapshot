...
def worse(args, prev, m, out, tmp, loo_detail=None, files_now=None):
    ...
    This comparison is only valid for a RE-FIT - same rectangle, more frames.
    It is invalid for a GEOMETRY change, where a lower leave-one-out can mean
    the new crop has stopped fitting noise the old one was fitting. Measured on
    dishwasher_door: tightening the rectangle to the band that actually differs
    between states took leave-one-out from 0.960 to 0.893 while reducing the
    model's swing on a stationary door from 0.676 to 0.003. Leave-one-out scores
    agreement with labels captured through the OLD rectangle; it cannot see that
    the rectangle contained the wrong thing. Use --geometry-change for that case
    and read both numbers rather than trusting the comparison (#45).
    ...
    """
    new = metric(m)
    if getattr(args, "geometry_change", False):
        # a different crop is a different question; the stored score answers the
        # old one. Report both and install, rather than silently keeping a model
        # that fits a rectangle somebody has deliberately changed.
        old = metric(prev) if out.exists() else None
        print(f"  geometry change: installing on the new crop "
              f"(leave-one-out {new if new is None else round(new, 4)} vs {old if old is None else round(old, 4)} on the OLD rectangle - not comparable)")
        return False
...
def main():
    ...
    ap.add_argument("--geometry-change", action="store_true", help="the zone rectangle changed: the crops have new content under the same names, so retrain even though the signature matches, and report both scores rather than comparing them")
    ...
    for d in sorted(p for p in root.iterdir() if p.is_dir() and "__" in p.name):
        ...
        if len(classes) >= 3:
            ...
            # A geometry change re-crops every file in place: same names, new
            # pixels, identical signature. Without this the tool reports
            # "unchanged" and skips, and the model in place goes on scoring a
            # crop it was never trained on - silently, every night (#45).
            if not args.force and not args.geometry_change and out.exists() and prev.get("counts") == counts and prev.get("sig") == sig:
                print(f"{d.name}: unchanged {counts}, trained {prev.get('trained')}")
                continue
            if not args.force and not args.geometry_change and (prev.get("rejected") or {}).get("sig") == sig:
