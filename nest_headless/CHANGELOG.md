...
- **A cat labelled from the backlog was invisible to matching until the next
  restart.** `sampleItem` builds the in-memory gallery entry and did not carry
  `v`; `loadEnrolled` added it afterwards and `writePersonSample` did not. So a
  freshly labelled sample entered `enrolledCats` with `v: undefined`, and
  `currentCats()` - which filters on `it.v === catid.VERSION` - retired it on
  arrival. The record on disk was correct throughout; only the index was wrong,
  and a restart fixed it by re-reading the file, which is why it was never
  noticed: deploys restart the process often enough to mask it.
  Caught by watching a real labelling land and seeing the gallery's `stale`
  count rise instead of `current` (#23).
- Same shape as 1.31.2's pending-store whitelist, one layer up: a field named
  in one construction path and omitted in the other, with the two never
  compared. `sampleItem` now carries `v` and `sized` itself, so both call sites
  get them by construction rather than by remembering.

## 1.33.2

- **The zone log line named one label and printed another's score.** The payload
  has carried the settled state's own score since 1.23.1; the log a person
  actually reads still printed `v.score`, the argmax's. On a three-label zone
  that produced `-> closed (score 0.741)` where 0.741 belonged to `vent` and
  `closed` had scored 0.235 - the exact fault that was fixed in the payload,
  left behind in the line that gets quoted in issues. It now prints the settled
  state's own score, and says so explicitly when this tick disagrees:
  `-> closed (score 0.235, unsettled - this tick reads vent 0.741)`.

## 1.33.1

- **`verdict_score` reports the confidence in the label `verdict` names.** It
  carried the raw positive-class score, which on a two-label zone is P(open)
  whatever the verdict is - so a closed verdict reported the open probability:
  `state=closed score=0.998 scores={open 0.002, closed 0.998}
  verdict_score=0.002`. `score` and `scores` were corrected in 1.33.0 and needed
  no further change; the reporter re-measured and narrowed their own complaint
  to this field alone (#52). Three-label zones already named their own label and
  are unaffected.
- For the record, since it was asked: **`state` is selected by the debounce, not
  by the argmax.** A flip needs `zone_debounce_ticks` consecutive agreeing ticks,
  so `state` lags `verdict` by design, and `settled` is exactly the field that
  reports whether the two currently agree. Both binary models run at threshold
  0.5, so `verdict` *is* the argmax for them - a `state` that differs from the
  argmax is the debounce holding, not a different selection rule.

## 1.33.0

- **A two-label zone reading `closed` reported the other label's confidence.**
  `state: closed, verdict: closed, settled: true, verdict_score: 0,
  scores: {"open": 1, "closed": 0}` - nine events of that shape. A binary
  model's `score` is already P(open) and `positive` is derived from it, so
  branching on `positive` inverted the pair on the closed side. On an open
  reading the conditional gave the right answer by coincidence, which is why
  only `closed` ever showed it and why every model-accuracy check passed while
  a consumer saw something wrong. Three-label zones were never affected. The
  cost was not cosmetic: a consumer gating certainty on `scores[state]` read
  every two-label close as "0.00 sure of closed" and could never be certain a
  fridge was shut (#52).
- Documented that **`t` is when the house was told, not when it happened**. A
  zone announces at most once per `zone_min_dwell_s` (60 s), so consecutive
  announcements are always at least 60 s apart and any duration derived from
  `t` is quantised to 60. `previous_duration_s` is the honest number and
  measures from when the previous state began - over 717 announced transitions
  it runs 6, 8, 10, 12 s at the head and only 7 land on 59-60. Also spelled out
  that `duration_resolution_s` is the resolution of `previous_duration_s` and
  not of `t`, and that there is no maximum visit length - only a cap on how
  often the house is told (#52).

## 1.32.2

- **The per-camera ONNX classifier keeps the promise its own doc comment
  makes.** `classifyDoor` is documented as returning "null when no model /
  failure" and every caller is written to that, but the load underneath threw -
  so a corrupt or half-written model file rejected the zone tick instead of
  answering null. That is not hypothetical: retrained models are written to a
  share while the add-on is running, so a half-written file is a state the
  system passes through by design. A failed load now warns once, answers null,
  and is retried when the file changes or after 30 s.

## 1.32.1

- **A model that fails to load is retried; a model that is absent is not.** The
  same shape as 1.31.1's browser bug, found by auditing for it: the cat
  detector, the object detector and the face models each cached their load
  result for the life of the process, and each turned a failure into a falsy
  value rather than a throw. So one bad moment on the network share the models
  live on - and that share has already produced stale reads and `EBUSY`
  renames - disabled detection or face recognition until the next restart,
  silently, because every call site is `if (!s) return null` and carried on
  without one. A throw now clears the cache and is retried after 30 s, which is
  long enough not to hammer a share that is genuinely gone. Absence stays
  sticky: there really is no model, and re-checking every frame buys nothing.

## 1.32.0

Both halves of last night's outage, fixed at the cause rather than the symptom.

- **A standby with no cameras is no longer promoted over an instance that can
  see.** The deadline was the only rule: wait up to 150 s for the standby's
  watches, then take over regardless. That promoted an instance with
  `0/2 watches live` and the house went blind for three minutes. At the
  deadline the standby now re-checks the outgoing instance: if it still has a
  live camera and the standby has none, the standby stands down and exits, and
  the supervisor keeps the old one - machinery that already existed for a
  standby that dies. Parity is deliberately **not** required: one camera of two
  still promotes, because a camera the vendor app has switched off must not
  hold up every deploy for ever. Only zero is refused.
  `HANDOVER_ALLOW_BLIND=true` overrides. The decision is a pure function in
  `app/handover.js` with tests for each case.
- **A standby brings its cameras up before warming its vision models.** Warming
  compiles ONNX graphs and holds the event loop; measured at 47 s during a
  handover with the loop lagging 78 s behind it, against 2.9 s on an idle boot.
  Puppeteer's 30 s wait for Chrome's endpoint URL is counted on that same loop,
  so the browser launch "timed out" while Chrome was starting perfectly well -
  which is what left the standby blind in the first place. A standby needs its
  cameras before it needs to recognise anything, so the heavy work now waits
  for the first watch, or `standby_warm_cap_s` (120) if the cameras never
  arrive. A normal boot is unchanged.

## 1.31.2

- **1.30.1's fix was a no-op and this makes it real.** `pending.add` builds its
  metadata from an explicit field whitelist rather than a spread, so `box`,
  `sized` and `v` were passed in by the caller and silently discarded by the
  store. The call site was updated and the consumer was updated; the thing in
  the middle was not. Caught by checking a real backlog entry written after the
  deploy, which still had none of the three.
- The half that mattered was already working, because `enrolPending` falls back
  to `v: m.v || catid.VERSION` - so cat samples labelled from the backlog were
  being stamped and were no longer retired the moment they were saved. What was
  still being lost is `box`, and with it the ability to ever re-describe a
  sample when the descriptor version moves on.
- Only carried when the caller supplies them, so face and voice entries keep
  their existing shape. Tested both ways.

## 1.31.1

- **A failed browser launch is no longer remembered forever.** `getBrowser()`
  cached the launch promise and only ever cleared it from the `disconnected`
  handler - which requires a launch to have *succeeded*. So one rejection was
  kept for the life of the process: every later call returned the same rejected
  promise without attempting anything. Both cameras stayed down with **zero**
  chromium processes, retrying every 30 s against a promise that never retried.
  The rejection now clears the cache so the next caller launches for real.
- The launch failed in the first place because the incoming instance starved
  its own event loop: `vision models warm in 47332 ms`, then
  `event loop lagging 78604 ms`. Puppeteer's 30 s wait for Chrome's endpoint is
  measured on that loop, so it expired while Chrome was in fact starting
  normally - the binary was fine and launched by hand in under a second.
- Both together turned a transient startup timeout into a standing outage, and
  the handover promoted the new instance anyway: `handover: proceeding with
  0/2 watches live after 151s`. Promoting an instance that can see nothing is
  its own fault and is not fixed here.

## 1.31.0

- `/speech/clip` honours what the calling device says about how it was woken.
  A screen that spots its own keyword, or that offers a button to talk, knew
  the answer before it uploaded anything - and the route was throwing that away
  and re-deriving the wake from the transcript alone. `wake_method: "spotter"`
  now admits a near-spelling in the transcript, the same evidence 1.29.0
  accepts on the camera path; `wake_method: "button"` (or tap/manual/press)
  confirms outright, because somebody pressing a key has stated intent more
  plainly than any phrase. Anything else, or absent, behaves exactly as before.
- The reply says **which** rule confirmed it: `wake.via` is `transcript`,
  `spotter_near_spelling` or `device`. The three are not equally strong -
  `device` never looked at the words at all - and a consumer weighing a
  borderline request needs to tell them apart rather than reading one boolean.
- `spotterSpelledDifferently` takes a `withinChars` window. A camera segment
  starts at the spotter fire so the phrase is at the front, and the 40-char
  default keeps that rule; an uploaded clip carries its whole start, so the
  phrase may sit anywhere in it.

## 1.30.1

- A cat labelled from the review backlog was written with **no descriptor
  version**, and `loadEnrolled` reads a missing version as v1 - so the sample
  was retired the moment it was saved. 36 of them had accumulated: two thirds
  of one cat's gallery and five eighths of another's, every one a current
  descriptor discarded for a missing field. `pending.add` kept the embedding
  but not the `v`, `sized` or `box` it was computed with, so nothing
  downstream could say what it was, and nothing could ever re-describe it
  either. All three now travel with the embedding and are stamped on the
  record (#23).
- A cat sample saved from the backlog was filed in the **face** gallery in
  memory until the next restart, because the map was chosen as "voice or
  faces". Corrected on reload, invisible until then.
- `/identity/<name>/samples` reported `used: true` for every sample, hardcoded.
  A gallery that had silently retired a third of its samples still showed all
  of them as in use, which is why this went unnoticed for a week. Cat samples
  now report their real version and whether they are actually in the gallery.

## 1.30.0

- A zone state change reports the faces seen across the **visit**, not one
  glance at the instant it is announced. `zone_change` already got the hold's
  accumulated faces and logged "visit by <name>"; `zone_state` - the event
  chore attribution actually reads - took a single `peopleNear` sample at the
  moment the state flipped. At a dishwasher that moment is exactly when the
  person is bent over the open door with their face to the floor, so the
  stream that decides whether a child is credited saw nobody while the stream
  that does not saw them plainly. Faces now merge on box overlap with the
  best-evidenced view winning, anything contributed by the visit is marked
  `from_visit`, and a visit's faces stay usable for `visit_faces_s` (90) after
  the visitor steps away so a flip announced just afterwards still carries who
  was there (#46). Everything contributed by one visit shares a `visit_id`: the
  hold merges on box overlap, so a person who steps aside and returns becomes
  two entries and a long visit can offer four names for one or two people. As
  independent boxes a consumer may pick the strongest and look confident; with
  a shared id it can see they are alternatives competing for one identity and
  hold them all as probable.

## 1.29.1

- A capture that never settles no longer wedges every later one. `/snapshot`
  and `/detect` timed out indefinitely while `/health` answered instantly and
  the watch loop carried on sensing, which is what made the process look
  healthy. The coalescing stores the in-flight promise and hands it to every
  subsequent caller, so a hung `watchGrab` - an unresponsive page, a detached
  frame - was permanent until restart. Both the grab and the whole capture are
  now bounded by `capture_timeout_s` (25), turning a wedge into one failed
  request (#51).

## 1.29.0

- A spotter wake also confirms when the transcript spells the name the
  recogniser's own way: "Hey Claw" for Claude, "Hey Oakbird" for Oakbeard. The
  test was an exact match against a transcript the recogniser does not spell
  that way, so a child said both live wake phrases in one breath and both were
  dropped (#49). Narrow by design: only on the spotter path, where the sound
  has already been matched; only after a **strong** wake word, because a soft
  "a" in front of an ordinary word is ordinary English ("a bottle" shares three
  letters with the name); and only a shared opening of three characters, not a
  similarity score - measured over the house's transcripts, similarity does not
  separate a real attempt from a sentence. Validated over every spotter fire in
  the log: it adds one, the child's, and no false ones.

## 1.28.0

- `people_nearby` from a held zone visit is one entry per **person**, not per
  (position, name). The dedupe key was "same box column AND same name", so one
  face read as one child on one 20 s sample and her sister on the next became two
  entries on nearly the same box - indistinguishable downstream from two
  sisters standing side by side, and a coin flip between them arrived as a
  certainty about one of them. Entries now merge on overlap
  (`HELD_SAME_PERSON_IOU` 0.4), keep the best-evidenced view of each person,
  and carry `matches` so a consumer can tell "this face is 0.517 person A or
  0.449 person B" from "A and B are both here". `name`/`score`
  remain the top match, so existing readers are unaffected (#43).

## 1.27.1

- `nest_headless_zone_state` carries `verdict`, `verdict_score` and `settled`,
  matching `zone_change` since 1.23.1, and its `score` is now the settled
  state's own score rather than the raw tick score - a payload could read
  `state: closed, score: 1` with the 1 belonging to `open`. The payload
  describes two instants (the transition, and the tick it was announced on,
  up to `zone_min_dwell_s` later) and nothing said which fields belonged to
  which; DOCS now does, and `settled` marks a zone mid-flip.

## 1.27.0

- `people_region` (off by default): scan the grown zone rather than the whole
  frame for people. A wide room view letterboxed into the detector's 640x640
  leaves a person about 120 px tall; cropped to the zone the same person is
  ~400 px. On this camera it recovered 8 of 8 real people against full frame's
  3, with no false positive across 170 negatives, at an unchanged 0.4.

## 1.26.1

- `POST /speech/clip` refuses above `clip_rate_max` (6) clips a minute per
  calling address, with `429` and `reason: "rate_limited"` plus
  `retry_after_s` and a `Retry-After` header. Refused before the audio is
  decoded, so a looping device costs nothing and never reaches the
  recogniser. The route shares Parakeet with two cameras that have brakes,
  and a device without one wins every race it should lose. The distinct
  reason matters at the screen: braked and unintelligible need different
  advice, and "say it again more clearly" is actively wrong when the answer
  is "wait four seconds".

## 1.26.0

- `POST /speech/clip`: one clip in, transcript out, and the speaker too with
  `identify: true`. Both run on the same prepared samples, so the row that
  names somebody carries the words they actually said. Same engine, wake rule
  and preprocessing as the camera path, and the same `engine` string on the
  response, so a screen and a camera that heard one sentence can be compared
  like with like. Loopback or bearer token. Nothing is kept and no event is
  emitted.

## 1.25.4

- A snapshot whose destination a network share holds open is written in place
  rather than renamed. `www/` is an SMB share that Home Assistant reads
  directly, and the hallway picture sat four hours stale while every rename
  returned `EBUSY` and the temp file was rewritten every second. Not atomic,
  so a reader can catch a half-written frame; one torn frame beats a picture
  that stops. Any other rename error still throws.

## 1.25.3

- All three `nest_headless_keyword` emitters carry both `source` and
  `confirmed`. The transcript path sent `source` but never `confirmed`,
  although it is only reached when the transcript already carried the wake
  phrase - so a consumer reading `confirmed` saw it absent on every
  transcript wake, and reported "confirmed 0" across every wake phrase in the
  house.

## 1.25.2

- A spotter hit is no longer discarded silently. `segPending` suppressed one
  for the whole recogniser round-trip, including phrases spoken after the
  pending audio had ended; it is now bounded by `seg_race_ms` (1200 ms),
  which covers the real race - the segment ends and the spotter, lagging the
  phrase by 0.3-0.7 s, fires on its tail. Every swallowed hit is counted per
  reason (`keywords_swallowed_capture_open`, `_segment_race`, `_repeat`) and
  logged at most every five minutes. A wake the house hears and does not act
  on is the one failure it could never explain to the person standing in it.

## 1.25.1

- `seg_rate_max` raised from 8 to 24 segments a camera a minute. Eight was
  set when three Parakeet workers shared an uncapped cache; at eight, 150-300
  segments a day were discarded, and a busy kitchen exceeds it exactly when
  somebody is most likely to be talking to the house.

## 1.25.0

- `POST /identity/voice/who` accepts `min_score` and `min_gap`, echoed back as
  `applied`. A shared device needs a stricter bar than a phone with an owner.

## 1.24.0

- `identity_excluded` and `exclude_suppress_at`: a person can ask not to be
  recognised, and the exclusion is applied where the naming happens rather
  than at the edges.

## 1.23.1

- `model` on `zone_change` reports `state`, `score`, `verdict`,
  `verdict_score`, `settled` and the full `scores` map together, so
  `model.state` and `model.scores` can no longer contradict each other.

## 1.23.0

- The best face over a track (`track_face_ms`, 1500) rather than the face in
  the crossing frame; `face_from` says which was used.

## 1.22.0

- A zone's debounce scales to its model's measured quality: 3 ticks at 0.95
  leave-one-out and above, 4 at 0.90, 8 otherwise (`zone_debounce_min_ticks`).

## 1.21.1

- The state duration is logged as well as sent.

## 1.21.0

- `zone_change` reports how long the previous state actually lasted, with
  `duration_resolution_s`, `measured` and `state_since`, instead of the gap
  between announcements.

## 1.20.5

- A picture the cat detector finds nothing in is only described whole when it
  is at least `photo_min_px` (256) on its short side.
...
- A quietly spoken "hey" (heard as "a" or "eh") only wakes the house before a
  ...
  "heart" and "lantn" cover the consumer, "cloud", "cord" and "god" cover Claude -
...
- A segment wakes on the phrase wherever it falls, not only in the first 40
  ...
  Oakbeard" repeatedly one evening and 810 of 811 segments were discarded.
...
- A house can name its assistant (#29): `wake_names` (the names accepted
  ...
  expects, e.g. `HEY OAK BEARD` -> `HEY OAKBEARD`). The spotter's own
...
- Activity zones ignore people (consumer #12 field note): before a zone goes
...
- Quiet speech (consumer #15): when the end-pointer would report `no_speech`
...
- State zones (consumer #12, #13), the senses/intelligence split made
...
- API authentication for the sensitive routes (consumer #10): `/listen`,
...
- `POST /identity/face/enrol` accepts a supplied image (consumer #9): JSON
...
- Classifier health made explicit (consumer #8): `classifier.state` is `ok`,
...
- Face identity (consumer #2, second half). SCRFD (10g) detection + 5-point
...
- Passage zones (consumer #7). `watch_passages` takes polygons drawn across
...
- Follow-up window (consumer #4): `POST /listen/<camera>?seconds=8` opens a
...
- Once speech has been heard, "quiet" is judged relative to that speech
  ...
  to the 8 s hard stop for a 3.5 s question (consumer #3, instrumented). The
...
- Utterances are peak-normalised (to -3 dBFS, at most x20) before
  ...
  0.01-0.08 and Whisper read it as noise (consumer #3).
...
- Speech noise floor = the ring's quietest tenth x3, clamped to 0.006-0.015:
  ...
  fallback now counts the run-on as speech already (consumer #3). Transcripts
...
- Whisper transcripts. Point `stt_model_dir` at a sherpa-onnx Whisper model
  ...
  Transcripts stay authoritative in the add-on (consumer #3). Hosts without a
...
- Speech end-pointing (consumer #3): the pre-roll and the first 300 ms after
...
- Voice identity (the consumer issue #2). Each `nest_headless_speech` utterance is
