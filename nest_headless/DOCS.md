...
| `wake_names` | (built-in list) | names accepted after the wake word, `|`-separated, spaces allowed (`oakbeard|oak beard|claude|clawed`) |
...
## `zone_change` is a pixel diff, not a transition

The two zone events answer different questions and only one of them is about a
change of state.

```
zone_change   the picture in this rectangle no longer matches the held
              reference frame. Fires on `diff >= zone_change_threshold`.
              Carries diff, reference_held_s and a before/after JPEG pair.
              NO state transition is being claimed.

zone_state    the debounced state label moved from A to B. This is the
              transition event.
```

The `model` block on a `zone_change` is context added later (#22) - a snapshot of
the model's current view at the moment the picture changed. Inside it,
`state` is the settled, debounced label, exactly as on `zone_state`.

**So `state: closed` beside `scores: {open: 1.0}` on a change event is not a
contradiction - it is the event doing its job.** It says *officially closed, and
the frame now looks open*. On a change event `state != argmax(scores)` is not
merely permitted, it is the **expected** case: the event fired *because* the view
diverged from the reference. A change event where the two agreed would be the
odd one.

**Do not take a transition's confidence off a change event.** No change of state
is being claimed there, so neither `score` nor `verdict_score` answers "how sure
are we this happened". Read `zone_state` for that, where `score` is the settled
state's own confidence.

### Which confidence goes with which label

```
score          <->  scores[state]     the settled label's own confidence
verdict_score  <->  scores[verdict]   this tick's raw argmax, its own confidence
settled        <->  whether those two labels are currently the same
```

`verdict_score == scores[state]` holds **only while `settled` is true**. Testing
that pairing against change events - the events that exist because things
diverged - will fail on every one of them and look like a fault. It is not one.

## `t` is when the house was told, not when it happened

A zone announces at most once per `zone_min_dwell_s` (60 s). A zone that flaps
falls silent rather than filling the house with contradictions (#26) - so
consecutive announcements for one zone are **always at least 60 s apart, by
construction**.

That makes `t` unusable as a clock for durations:

```
WRONG   duration = t(closed event) - t(open event)     -> quantised to >= 60 s
RIGHT   duration = previous_duration_s on the closing event
```

Measured over 717 announced transitions, `previous_duration_s` runs
6, 8, 10, 12, 14, 16, 18 s at the head of the distribution and only **7 of 717**
land on 59 or 60 s. A consumer deriving the same quantity from `t` sees the
opposite - a pile-up at 59-60 - and the difference is entirely the gate.

`previous_duration_s` measures from when the previous state **began** to when
this one began, not from one announcement to the next. That distinction is the
whole of #22: before 1.19.1 they were the same thing, and every fridge visit was
reported as exactly 60 s whether the door was open for five seconds or fifty.

`duration_resolution_s` (6) is the debounce - `zone_debounce_ticks` x
`zone_tick_ms` - which is how long a verdict must hold before the state moves at
all. It is the resolution of `previous_duration_s`. **It is not the resolution
of `t`**, and the two numbers being in one payload has misled a reader once
already: 59 is not a multiple of 6, which is a good sign that the number in hand
did not come from the field that declares that resolution.

There is **no maximum visit length**. A state that lasts an hour reports an hour.
What is capped is how often the house is *told*, never how long a state may run.

## Two moments in a zone_state payload

`nest_headless_zone_state` describes **two different instants** and both sets of
fields sit at the same level. Reading them as one moment produces a spurious
disagreement rate - on one 86-event sample, 32 events looked as though the model
contradicted its own label, and 29 of those were this.

```
the TRANSITION           state, previous, state_since, previous_duration_s
THIS tick                verdict, verdict_score, scores, frame_at, t
the settled state's score score
```

A state flip is held back while the zone settles (`zone_min_dwell_s`, 60 s), so
the announcement - and therefore `frame_at` - can be up to a minute after
`state_since`. By then the door may have moved again, which is why `verdict`
(this tick's raw argmax) can differ from `state` (the debounced label). When they
differ, `settled` is false; that is the zone mid-flip, not a fault.

`score` is the **settled state's own** score, matching `zone_change` since
1.23.1. Before 1.27.1 it was the raw tick score, so a payload could read
`state: closed, score: 1` where the 1 belonged to `open`.

To compare like with like, use `verdict` against `scores`, or `state` against
`score` - never `state` against `scores`.

## `name` is the claim; `matches` is a ranking

Every entry in `people_nearby` (and the `person`/`faces` blocks on a passage)
carries both, and they answer different questions:

```
name    who this is, or null      the add-on's claim. null means it will not say.
score   that name's score         null when name is null.
matches every candidate, ranked   a ranking, not an identification. Always present
                                  when a face was compared, INCLUDING when nothing
                                  cleared the bar.
```

`name` is set only at cosine **>= 0.4**. Below that the add-on reports
`name: null, score: null` and still returns the full `matches` list, because a
consumer deciding whether a room is ambiguous needs to see the runner-up. So
`matches[0]` existing does **not** mean anybody was identified.

**Read `name`. Use `matches` for ambiguity, never for identity.** On
2026-09-10 a household chore was credited to a child from a box this component
had explicitly declined to name — `name: null`, `score: null`,
`matches[0] = 0.396` — because the ranking was read as an answer. The contract
was doing what it should; the ranking simply travels further than it looks.

Three shapes and what each means:

```
name set, matches [...]      identified, with the runner-up available
name null, matches [...]     a face was compared and nothing cleared 0.4
name null, matches []        no face was matched into this person box at all
                             ("somebody is here and I cannot see who")
```

The third is common and is not a missing field: a person facing away, or too
small, has no face to compare. At a dishwasher it is the normal case, because
the act of loading one turns your face towards the floor.

### `visit_id` does not count people

A state change also reports faces seen during the **visit**, not only at the
instant the state flipped — at an appliance the moment of the flip is usually
the moment the person is bent over it with their face to the floor. Those
entries carry `from_visit: true` and a `visit_id`.

```
same visit_id        observations of ONE continuous presence at the zone.
                     Alternatives for one identity, not several people.
different visit_id   two separate presences. Says NOTHING about how many people.
no visit_id          seen at the moment the state flipped: the stronger evidence.
```

**Neither field establishes a count.** A visit ends after **6 seconds** of the
zone being clear (`STEADY_TICKS` 3 x `ZONE_TICK_MS` 2000), so one person opening
a fridge, stepping to the worktop and coming back produces two visits with two
ids. Consecutive fridge visits of 8 s, 6 s, 10 s minutes apart are one person
using a kitchen, not three people.

So: same id means *probably one identity*; different ids mean *we cannot tell*.
Reading two ids as two people is the mistake that would credit two children for
one person's chore.

Visit-derived faces are weaker evidence than a face seen at the flip — the
person was at the appliance at some point in those two minutes, not necessarily
when the door moved. They should be able to name somebody and should not be
enough to earn on their own.

## What the audio counters mean

`GET /` reports per-camera audio counters under `audio`. They are the only way
to tell a fault from the design working, and on 2026-09-08 all three sessions
working on this house read the same silence as a broken microphone when it was
none of them. **An absent event is not evidence of a deaf ear** - most speech is
deliberately discarded, and each counter says which kind.

```
chunks              audio chunks received (the stream is alive)
decodes             spotter decode passes (the spotter is running)
lastRms             level of the last chunk (the room is audible)
segments            segments of speech the segment path transcribed
segments_dropped    ...of those, discarded: no wake phrase, no open window
segment_wakes       ...of those, addressed to the house - these emit an event
segments_throttled  refused: over seg_rate_max a minute
segments_paused     refused: the recogniser servers were failing
segments_duplicate  the same voice already owned by another camera
hits                spotter keyword hits acted on
keywords_swallowed  spotter hits heard and NOT acted on, plus a reason:
                      _capture_open  a capture was already running
                      _segment_race  a transcript decision was in flight
                      _repeat        within 2.5 s of the last keyword
lastKeyword         what the spotter last matched, set before any guard
```

Reading them:

- **`segments` climbing with `segments_dropped` climbing equally is the system
  working.** A segment with no wake phrase and no open conversation window is
  dropped without an event, or every sentence spoken in the kitchen becomes a
  row. 551 of 552 dropped in an evening is normal.
- **No `nest_headless_speech` for a camera does not mean it heard nothing.** It
  means nothing was addressed to the house. Check `segments`, not the event bus.
- **`lastKeyword` set with `hits` at 0** means the spotter matched and something
  refused it; `keywords_swallowed_*` says which guard.
- **`lastKeyword` null** means the spotter never matched at all - a different
  problem from a match that was discarded, and worth separating before tuning
  anything.
- **`chunks` climbing with `decodes` flat** would mean the spotter is not
  running; `lastError` says why.
...
`http://127.0.0.1:8098/` and the consumers (the consumer) should use that address.
