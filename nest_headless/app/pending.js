...
class PendingStore {
  ...
  add(sample) {
    ...
    // This is a whitelist, not a spread, so a field the caller adds is silently
    // dropped unless it is named here. That is what happened to box/sized/v in
    // 1.30.1: the call site was updated and the consumer was updated, and the
    // store in the middle quietly discarded them (2026-09-10). Only carried
    // when present, so face and voice entries keep their existing shape.
    const meta = { id, kind: sample.kind, camera: sample.camera, t: sample.t || new Date().toISOString(), utterance_id: sample.utterance_id || null,
      ...
      ...(sample.box ? { box: sample.box } : {}), ...(sample.sized != null ? { sized: !!sample.sized } : {}), ...(sample.v != null ? { v: sample.v } : {}) };
