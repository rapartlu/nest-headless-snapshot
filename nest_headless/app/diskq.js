...
function writeAtomic(file, buf) {
  ...
  entry.promise = serial(file, async () => {
    ...
    try {
      ...
      // A network share refuses to rename onto a file another host holds open.
      // Home Assistant reads these snapshots straight out of www/, and on
      // 2026-09-07 the hallway picture sat four hours stale while every rename
      // returned EBUSY - the temp file was rewritten every second and never
      // arrived. Writing in place is not atomic, so a reader can catch a
      // half-written frame; one torn frame beats a picture that stops.
      if (e.code !== 'EBUSY' && e.code !== 'EPERM' && e.code !== 'EACCES') throw e;
      await fsp.writeFile(file, entry.buf);
