...
// Keep in lockstep with config.yaml `version` - consumers (the consumer) read it
// from GET / to detect that a deploy has landed.
const ADDON_VERSION = '1.33.3';
...
const cfg = {
  ...
  // Classify zones (consumer #12): named crops with a model each
  ...
  // /audiodebug) from anywhere but loopback (consumer #10). API_TOKEN or
...
async function getBrowser() {
  if (!browserPromise) {
    ...
    browserPromise = puppeteer.launch({
      ...
      // A FAILED launch must not be cached. browserPromise was only ever
      // cleared by the 'disconnected' handler, which needs a launch to have
      // succeeded first - so one rejection was remembered for the life of the
      // process and every later getBrowser() returned it without trying again.
      // Both cameras stayed down with zero chromium processes, retrying every
      // 30 s against a promise that never re-attempted anything (2026-09-10).
      browserPromise = null;
      setInferencePriority(true);
      throw e;
    });
    ...
    if (shot.cropDataUrl) {
      ...
      if (infer.hasDoorModel(entityId)) {
        ...
        if (cnn) {
          ...
          // framingOk is stale noise here (consumer #8): not reported.
          verdict = { ...cnn, positive: cnn.positive };
        }
      }
      // Explicit health for consumers (consumer #8): ok | dark | framing_drift
...
// A capture that never settles used to wedge every later one (#51). The
// coalescing is the point - one Google command for concurrent callers - but it
// stores the in-flight promise and hands it to everybody who asks next, so a
// hung `watchGrab` (an unresponsive page, a detached frame) meant `/snapshot`
// and `/detect` timed out for good while the watch loop carried on. `/health`
// answered instantly throughout, which is what made it look like the process
// was fine. A bounded capture turns a permanent wedge into one failed request.
const CAPTURE_TIMEOUT_MS = intEnv('CAPTURE_TIMEOUT_S', 25) * 1000;
function withTimeout(p, ms, what) {
  let t;
  return Promise.race([
    Promise.resolve(p).finally(() => clearTimeout(t)),
    new Promise((_, rej) => { t = setTimeout(() => rej(new Error(`${what} timed out after ${ms} ms`)), ms); if (t.unref) t.unref(); }),
  ]);
}
async function captureCoalesced(entityId) {
  ...
  s.inflight = withTimeout((async () => {
    ...
    const w = await withTimeout(watchGrab(entityId), CAPTURE_TIMEOUT_MS, `watchGrab ${entityId}`).catch(() => null);
    return w || capture(entityId);
  })(), CAPTURE_TIMEOUT_MS, `capture ${entityId}`)
    .then((r) => { s.lastCaptureMs = Date.now(); s.lastMeta = r.meta; return r; })
    .catch((e) => { console.warn(`[nest_headless] capture ${entityId} failed: ${e.message}`); throw e; })
...
// stay responsive (consumer #17: a 20-minute 100% spin during an onboarding
...
// How often faces are sampled for tracked people between crossings (#35).
const TRACK_FACE_MS = intEnv('TRACK_FACE_MS', 1500);
// Passage tracking (consumer #7): every doorway-motion hit runs the person
...
async function passageTick(entityId, mgr, frameBuf, now) {
  ...
  try {
    ...
    // Best face over the track, not just the crossing frame (#35). Faces used
    // to be looked for only at the instant a line was crossed, which optimises
    // for the wrong moment: someone walking a hallway looking at their phone
    // presents a face for part of it and not at others. Sampled on its own
    // pacing while anyone is tracked, kept per track, and used at the crossing
    // only when that frame has nothing of its own.
    mgr.trackFaces = mgr.trackFaces || new Map();
    if (persons.length && faces.hasModels(FACE_MODELS_DIR()) && now - (mgr.lastTrackFaceMs || 0) >= TRACK_FACE_MS) {
      mgr.lastTrackFaceMs = now;
      try {
        const found = (await faces.facesInJpeg(FACE_MODELS_DIR(), frameBuf, { minPx: 40 })) || [];
        for (const f of found.map(faceSummary)) {
          const t = mgr.tracker.tracks.find((x) => f.box.x + f.box.w / 2 > x.box.x && f.box.x + f.box.w / 2 < x.box.x + x.box.w
            && f.box.y + f.box.h / 2 > x.box.y && f.box.y + f.box.h / 2 < x.box.y + x.box.h);
          if (!t) continue;
          const kept = mgr.trackFaces.get(t.id);
          // a named face beats an unnamed one; otherwise the bigger crop wins
          const better = !kept || (f.name && !kept.name) || ((!!f.name === !!kept.name) && (f.size_px || 0) > (kept.size_px || 0));
          if (better) mgr.trackFaces.set(t.id, { ...f, at: new Date(now).toISOString() });
        }
      } catch (e) { /* faces are a bonus */ }
      const live = new Set(mgr.tracker.tracks.map((t) => t.id));
      for (const id of [...mgr.trackFaces.keys()]) if (!live.has(id)) mgr.trackFaces.delete(id);
    }
    ...
    for (const ev of events) {
      ...
      const crossingFace = seen.find(inBox) || (seen.length === 1 && persons.length === 1 ? seen[0] : null);
      const tracked = mgr.trackFaces && mgr.trackFaces.get(ev.track_id);
      // this frame first; the track's best only when this frame has nothing
      const face = crossingFace || tracked || null;
      if (face) {
        ev.person = { ...ev.person, matches: face.matches, name: face.name, score: face.score, size_px: face.size_px };
        ev.face_from = crossingFace ? 'crossing' : 'track';
        if (!crossingFace && face.at) ev.face_at = face.at;
      }
      ev.faces = seen;
      ...
      console.log(`[nest_headless] PASSAGE ${ev.direction} ${ev.passage} on ${entityId} track ${ev.track_id} h=${ev.attributes.height_ratio} carrying=${ev.attributes.carrying} who=${ev.person.matches[0] ? ev.person.matches[0].name + ':' + ev.person.matches[0].score : '-'} faces=${seen.length}${ev.face_from === 'track' ? ' (face from the track)' : ''}`);
...
const ZONE_TICK_MS = 2000, STEADY_TICKS = 3, CHANGE_TICKS = 2, ZONE_CHANGE_THRESHOLD = 10;
// A state flip needs this many consecutive agreeing ticks. It was 2 (4 s),
// which any unsteady model clears: the three-state dishwasher model changed
// its mind 69 times in 70 readings on a door nobody touched, several in the
...
const ZONE_MIN_DWELL_MS = intEnv('ZONE_MIN_DWELL_S', 60) * 1000;
let sharpMod = null;
...
// Scan the grown zone instead of the whole frame (1.27.0, off by default).
// The camera is a wide 1920x1080 room view letterboxed into the detector's
// 640x640, which leaves a person 19% of frame height about 120 px tall - near
// the floor of what yolo11n can do. Cropped to the zone, the same person is
// ~400 px. Measured on this camera, at the unchanged 0.4 bar:
//   09-08 07:45  teenager, backpack   full: missed (0.224)  region: 0.794
//   09-08 06:36  child at the fridge  full: nothing @0.05   region: 0.896
//   09-07 19:21  two adults           full: missed          region: 0.733, 0.66
//   126 negatives (120 overnight + 6 with an independent people:0)
//                                     full: 1               region: 1  (same real person)
// Note the region-zoom warning in infer.js is about cats.onnx, which is
// house-trained on FULL frames and genuinely breaks when zoomed. yolo11n is
// stock COCO, trained across all scales, and has no such constraint.
// Default off: this changes who the house believes is in a room.
const PEOPLE_REGION = String(process.env.PEOPLE_REGION || 'false') === 'true';
async function peopleNear(entityId, jpg, rect) {
  ...
  try {
    const grown = { x: rect.x - rect.w / 2, y: rect.y - rect.h / 2, w: rect.w * 2, h: rect.h * 2 };
    const d = await infer.detect(jpg, { conf: 0.4, classes: [0], ...(PEOPLE_REGION ? { region: grown } : {}) });
    if (!d) return [];
...
// Two person boxes overlapping this much across a visit are taken to be one
// person seen twice, not two people (#43).
const HELD_SAME_PERSON_IOU = 0.4;
// How long a visit's faces stay usable after the visitor steps away (#46).
// zone_state used to take ONE peopleNear sample at the instant a state flip is
// announced. At a dishwasher that instant is exactly when the person is bent
// over the open door with their face to the floor, so the event that decides
// whether a child is credited saw nobody, while zone_change - which gets the
// hold's accumulated faces - logged "visit by <name>" for the same two minutes.
const VISIT_FACES_MS = intEnv('VISIT_FACES_S', 90) * 1000;
...
const iouBox = (a, b) => { const i = interArea(a, b); return i / Math.max(1e-9, a.w * a.h + b.w * b.h - i); };
...
function emitZoneChange(entityId, z, st, c, jpg, sharp, now, people, extra = {}) {
  ...
  postHaEvent('nest_headless_zone_change', {
    ...
    // The model's view, with the two clocks kept apart (#37). `state` is the
    // settled state and only moves after the debounce agrees; `scores` is the
    // latest tick. They disagree for the length of the debounce, and `score`
    // used to carry the winning value under the settled label - so a zone read
    // `state: open, score: 0.959` while 0.959 was in fact `closed`. `score` is
    // now the settled state's own score, and this tick's raw argmax is
    // reported separately as `verdict`.
    model: st.model ? {
      state: st.state,
      score: (st.scores && st.state != null && st.scores[st.state] != null) ? st.scores[st.state] : st.score,
      // the confidence in the label `verdict` NAMES, not the raw positive-class
      // score: on a two-label zone those are the same number only when the
      // verdict is `open`, so a closed verdict reported the open probability (#52)
      verdict: st.verdict || null,
      verdict_score: (st.scores && st.verdict && st.scores[st.verdict] != null) ? st.scores[st.verdict] : st.score,
      settled: st.verdict == null || st.verdict === st.state,
      scores: st.scores || null, engine: st.engine, ...(st.modelMeta || {}),
    } : null,
...
async function zoneClassifyTick(entityId, mgr) {
  ...
  for (const z of zones) {
    ...
    if (holdNow) {
      if (!st.heldSince) { st.heldSince = now; st.heldPeople = []; st.heldFaceMs = 0; st.visitId = `${entityId.replace(/^camera\./, '')}-${z.name}-${now}`; }
      if (now - st.heldFaceMs > 20000) {   // who is there, sampled at most every 20 s
        st.heldFaceMs = now;
        // One entry per PERSON, not per (position, name) (#43). The old key was
        // `same box column AND same name`, so one face the matcher read as
        // one child on one 20 s sample and her sister on the next became two
        // entries on nearly the same box - indistinguishable downstream from
        // two sisters standing side by side, and a coin flip between them
        // arrived as a certainty about one. Merge on overlap instead, keep the
        // best-evidenced view of each person, and carry `matches` through so a
        // consumer can see the ambiguity is within one face.
        peopleNear(entityId, jpg, c.rect).then((pp) => {
          for (const p of pp) {
            if (!p.box) continue;
            const same = st.heldPeople.find((q) => q.box && iouBox(q.box, p.box) >= HELD_SAME_PERSON_IOU);
            if (!same) { st.heldPeople.push({ name: p.name, score: p.score, box: p.box, height_ratio: p.height_ratio, matches: p.matches || [] }); continue; }
            if ((p.score || 0) > (same.score || 0)) { same.name = p.name; same.score = p.score; same.box = p.box; same.height_ratio = p.height_ratio; same.matches = p.matches || []; }
          }
        }).catch(() => {});
      }
      ...
      if (st.clearTicks >= STEADY_TICKS) {
        ...
        // The visit's faces outlive the visit for VISIT_FACES_MS (#46): a state
        // flip announced just after somebody steps back still carries who was
        // at the appliance, instead of one glance at an empty rect.
        if ((st.heldPeople || []).length) { st.recentPeople = st.heldPeople; st.recentPeopleMs = now; st.recentVisitId = st.visitId; }
    ...
    // A binary model's `score` IS P(open) - `positive` is derived from it, not a
    // separate fact - so the branch on `positive` inverted the pair whenever the
    // door was closed: score 0 became {open: 1, closed: 0} beside state
    // "closed". On an open reading the conditional happened to give the right
    // answer, which is why only the closed branch ever showed it, and why every
    // internal accuracy check passed while a consumer gating certainty on
    // scores[state] could never be sure a fridge was shut (#52).
    st.scores = v.scores || { open: Math.round(v.score * 1000) / 1000, closed: Math.round((1 - v.score) * 1000) / 1000 };
    const needTicks = debounceTicksFor(st.modelMeta);
    st.verdict = verdict;   // this tick's raw argmax, before the debounce (#37)
    ...
    // the house with contradictions (#26).
    if (st.state && st.state !== st.announced) {
      const quietFor = now - (st.announcedMs || 0);
      if (st.announced && quietFor < ZONE_MIN_DWELL_MS) {
        if (!st.flapLogged || now - st.flapLogged > 300000) {
          st.flapLogged = now;
          console.log(`[nest_headless] ZONE ${z.name} on ${entityId}: ${st.announced} -> ${st.state} held back, ${Math.round(quietFor / 1000)}s since the last word (settling)`);
        }
        continue;
      }
      ...
      // the SETTLED state's own score, as the payload has carried since 1.23.1.
      // `v.score` is the argmax's score, so on a three-label zone the line read
      // `-> closed (score 0.741)` while 0.741 belonged to `vent` - the same fault
      // the payload was fixed for, left behind in the log a reader actually sees.
      const shownScore = (st.scores && st.state != null && st.scores[st.state] != null) ? st.scores[st.state] : v.score;
      console.log(`[nest_headless] ZONE ${z.name} on ${entityId}: ${prev || 'unknown'} -> ${nextForEvent} (score ${shownScore}${st.verdict && st.verdict !== st.state ? `, unsettled - this tick reads ${st.verdict} ${st.scores[st.verdict]}` : ''}${durTxt}${trusted ? '' : ', model below trust floor - not announced'})`);
      if (prev && trusted) {
        const people = peopleForState(await peopleNear(entityId, jpg, c.rect),
          { held: st.heldPeople || [], recent: st.recentPeople || [], recentAtMs: st.recentPeopleMs || 0, now,
            windowMs: VISIT_FACES_MS, minIou: HELD_SAME_PERSON_IOU, visitId: st.visitId || null, recentVisitId: st.recentVisitId || null });
        postHaEvent('nest_headless_zone_state', {
          entity_id: entityId, camera: entityId.replace(/^camera\./, ''), zone: z.name, label: z.name,
          // Two moments in one payload, now labelled (#42). state / previous /
          // state_since / previous_duration_s describe the TRANSITION. verdict /
          // verdict_score / scores / frame_at describe THIS tick, which can be up
          // to zone_min_dwell_s later: a flip is held back while the zone settles,
          // and by the time it is announced the door may have moved again. `score`
          // is the settled state's own score, as on zone_change since 1.23.1 -
          // it used to be the raw tick score, so a payload could read
          // `state: closed, score: 1` with 1 belonging to `open`.
          state: nextForEvent, previous: prev, previous_duration_s: prevDuration,
          score: (st.scores && st.state != null && st.scores[st.state] != null) ? st.scores[st.state] : v.score,
          verdict: st.verdict || null,
          verdict_score: (st.scores && st.verdict && st.scores[st.verdict] != null) ? st.scores[st.verdict] : v.score,
          settled: st.verdict == null || st.verdict === st.state,
...
// scores; the brain (the consumer) owns consent and the decision.
let spkCtx = null;
...
function sampleItem(file, j) {
  // `v` and `sized` travel WITH the item. currentCats() filters on `v`, so an
  // item that arrives without one is retired the moment it is added - and
  // loadEnrolled used to add them on top while writePersonSample did not, so a
  // cat labelled from the backlog was invisible to matching until the next
  // restart. Same shape as the pending-store whitelist fixed in 1.31.2: the
  // record on disk was right and the index in memory dropped a field (#23).
    ...
    for (const name of fs.readdirSync(cfg.identityDir)) {
      ...
      for (const f of fs.readdirSync(d)) {
        ...
        try {
          ...
          if (Array.isArray(j.embedding)) ({ voice: items, face: fitems, cat: citems })[kind[1]].push(sampleItem(f, j));   // sized/v now come from sampleItem itself
        } catch (e) { /* skip */ }
      }
...
// ---- people who have asked not to be recognised (#38)
//
// A rule that is written down and not consulted is the failure pattern this
// house keeps hitting: an opt-out list was built for the fridge tracker and
// left empty, and a child was named on eleven fridge visits after asking not
// to be. So exclusion happens HERE, where the name is decided, not downstream.
//
// An excluded person is not merely unnamed. Their match is dropped, no name
// reaches an event, and nothing of theirs is parked for review - because a
// face that cannot be matched would otherwise be parked as a stranger, which
// is the same recognition by another route. The suppression floor is well
// below the naming line deliberately: the question is "is this probably
// them", not "are we sure".
const IDENTITY_EXCLUDED = new Set(String(process.env.IDENTITY_EXCLUDED || '')
  .split(/[|,]/).map((n) => n.trim().toLowerCase()).filter(Boolean));
const EXCLUDE_SUPPRESS_AT = numEnv('EXCLUDE_SUPPRESS_AT', 0.3);
const lastExcludeLog = {};
// `all` is the full ranked list including excluded people. Returns a list with
// them removed, carrying `.excluded` when one of them was the best answer.
function applyExclusions(all, kind) {
  if (!IDENTITY_EXCLUDED.size) return all;
  const top = all[0];
  const out = all.filter((m) => !IDENTITY_EXCLUDED.has(String(m.name).toLowerCase()));
  if (top && IDENTITY_EXCLUDED.has(String(top.name).toLowerCase()) && top.score >= EXCLUDE_SUPPRESS_AT) {
    const now = Date.now();
    if (now - (lastExcludeLog[top.name] || 0) > 60000) {
      lastExcludeLog[top.name] = now;
      console.log(`[nest_headless] IDENTITY ${kind} suppressed: ${top.name} has asked not to be recognised (would have scored ${top.score})`);
    }
    const empty = [];
    empty.excluded = top.name;
    return empty;
  }
  return out;
}
function matchFace(embedding) {
  ...
  return applyExclusions(out.sort((a, b) => b.score - a.score).slice(0, 3), 'face');
}
  ...
  for (const x of d.filter((det) => det.cls === 15 || det.cls === 16)) {
    try {
      ...
      out.push({ box: x.box, conf: x.conf, roi: x.roi || null, quality: { size_px: desc.size_px, det_score: x.conf, reason: usable ? 'ok' : 'cat_too_small' },
        name: top ? top.name : null, score: matches[0] ? matches[0].score : null, matches, _emb: usable ? desc.vec : null, _sized: desc.sized, _v: desc.v, _crop: desc.crop });
...
async function identifyCats(entityId, jpg, dets) {
  ...
  for (const c of r.cats) {
    ...
    try {
      const meta = pending.add({ kind: 'cat', camera: entityId.replace(/^camera\./, ''), t: new Date().toISOString(), quality: c.quality, matches: c.matches,
        // box, sized and v travel with the embedding. Without them the label
        // written later cannot say which descriptor produced it, and loadEnrolled
        // reads a missing version as v1 - which quietly retired 36 good samples.
        embedding: c._emb, box: c.box, sized: c._sized, v: c._v, size_px: c.quality.size_px, media: { buf: c._crop, ext: 'jpg' } });
...
// ------------------------------------------------------------ verification backlog (consumer #16)
...
function considerVoiceForPending(entityId, u, matches, utteranceId) {
  try {
    if (matches && matches.excluded) return;   // asked not to be recognised (#38)
...
async function considerFacesForPending(entityId, jpg, found) {
  ...
  try {
    ...
    for (const f of found || []) {
      ...
      if (matches.excluded) continue;   // asked not to be recognised: not named, and not parked either (#38)
...
function writePersonSample(name, kind, record, mediaSrc) {
  ...
  if (!enrolled) loadEnrolled();
  const map = kind === 'voice' ? enrolled : kind === 'cat' ? enrolledCats : enrolledFaces;
...
function enrolPending(id, name) {
  ...
  writePersonSample(name, m.kind, { camera: m.camera, source: 'room', quality: m.quality, embedding: m.embedding,
    ...(m.box ? { box: m.box } : {}), ...(m.sized != null ? { sized: !!m.sized } : {}),
    ...(m.kind === 'cat' ? { v: m.v || catid.VERSION } : {}),
    from_pending: id, matches_at_label: m.matches }, media && media.file);
  pending.remove(id);
...
function listSamples(name) {
  ...
  for (const f of [...files].filter((x) => /^(voice|face|cat)-.*\.json$/.test(x)).sort()) {
    ...
    out.push({ id, kind, t: j.at || null, source: j.auto ? 'auto' : (j.source || (String(j.camera || '').startsWith('camera.') ? 'room' : 'upload')),
      ...
      media_url: media ? `/identity/${kind === 'cat' ? 'cat/' : ''}${name}/samples/${id}/media` : null,
      // `used` was hardcoded true, so a gallery that had retired a third of its
      // samples still showed every one as in use. Only cats carry a version.
      used: kind !== 'cat' || (j.v || 1) === catid.VERSION, v: kind === 'cat' ? (j.v || 1) : undefined });
  }
  return out;
}
...
// The caller may set a stricter bar than the dictation default (#38). A shared
// screen anyone can pick up is not a phone in one person's hand: the phone can
// afford to guess because it is usually right about whose hand it is in, and a
// kitchen iPad cannot. Across 88 identifications only about a fifth reach the
// default 0.6/0.1, so on a shared device that bar attributes a fifth of
// interactions and the failure that matters is the wrong fifth - a child
// credited with an adult's request. The bar returned is echoed back so the
// caller can see which one was applied.
function identifyVoiceClip(samplesIn, { minScore = null, minGap = null } = {}) {
  ...
  const at = Number.isFinite(minScore) ? minScore : VOICE_DECISIVE;
  const gap = Number.isFinite(minGap) ? minGap : AMBIGUOUS_GAP;
  const decisive = !!matches.length && matches[0].score >= at && !(matches[1] && matches[0].score - matches[1].score < gap);
  return { ok: true, quality, matches, decisive, applied: { min_score: at, min_gap: gap } };
}
...
function matchVoice(embedding) {
  ...
  return applyExclusions(out.sort((a, b) => b.score - a.score).slice(0, 3), 'voice');
}
...
// opts.followUp (consumer #4): a listening window opened by the brain right
...
// End-pointing (consumer #3). The spotter fires mid-phrase ("hey cl..."), so
...
function feedSpeechCapture(entityId, chunk) {
  ...
  if (c.phase === 'tail') {
    ...
    // hard stop (consumer #3, 8 s for a 3.5 s question). The absolute floor
...
// ("HEY OAK BEARD") and the recognisers split unfamiliar names ("oak bird");
// WAKE_CANON maps aliases to the name the brain expects, e.g.
// "oakbeard=oak beard|oak bird;claude=clawed|cloud". Applied to both paths.
...
// the whole segment is dropped: a child said "hey Oakbeard" repeatedly one
...
// Some spellings in WAKE_NAMES are ordinary English ("heart" and "lantn" for
// the consumer, "cloud"/"cord"/"god" for Claude). Paired with a soft wake word they
...
const WAKE_NAMES_PLAIN = new Set(['heart', 'hart', 'lantn', 'cord', 'god', 'cloud', 'cloudy', 'clause', 'claws', 'clod']);
...
async function finishSpeechCapture(entityId, c, reason) {
  if (c.followUp && reason === 'no_speech') {   // nobody replied: no event at all (consumer #4)
  ...
  // Quiet speech (consumer #15): a child across the kitchen speaks at rms
  ...
  // A spotter capture also confirms when the transcript spells the name the
  // recogniser's own way - "Hey Claw" for Claude, "Hey Oabbe" for Oakbeard
  // (#49). Only on the spotter path: the sound has already been matched, so
  // the question is just whether this is that name written differently.
  const wakeConfirmed = !!text && !plainEnglishWake(text) && (
    WAKE_RE.test(text)
    || (c.fromSegment ? WAKE_ANYWHERE_RE.test(text) : WAKE_HEAD_RE.test(text))
    || (!c.fromSegment && wakeword.spotterSpelledDifferently(c.keyword, text, WAKE_WORDS_STRONG)));
  ...
  if (c.fromSegment) {
    ...
    if (wakeConfirmed && !c.followUp) {
      ...
      // confirmed by construction: this branch is only reached when the
      // transcript already carried the wake phrase (see wakeConfirmed above)
      postHaEvent('nest_headless_keyword', { entity_id: entityId, camera: entityId.replace(/^camera\./, ''), keyword: c.keyword, source: 'transcript', confirmed: true }).catch(() => {});
  ...
  if (KEYWORD_NEEDS_TRANSCRIPT && !c.fromSegment && !c.followUp && wakeConfirmed && c.keyword) {
    postHaEvent('nest_headless_keyword', {
      entity_id: entityId, camera: entityId.replace(/^camera\./, ''), keyword: c.keyword, source: 'spotter', confirmed: true,
    }).catch(() => {});
  }
  if (c.followUp && !text) {   // a follow-up window that caught only noise is the same as nobody replying (consumer #4)
...
// A spotter hit the add-on decided not to act on. Counted per reason and
// logged at most every 5 minutes per camera, so "it heard me and did
// nothing" is answerable from the log instead of being invisible.
const kwSwallowLog = {};
function swallowedKeyword(entityId, keyword, why) {
  segStat(entityId, 'keywords_swallowed');
  segStat(entityId, 'keywords_swallowed_' + why);
  const now = Date.now();
  if (now - (kwSwallowLog[entityId] || 0) < 300000) return;
  kwSwallowLog[entityId] = now;
  const s = audioStats[entityId] || {};
  console.log(`[nest_headless] KEYWORD "${keyword}" on ${entityId} heard but not acted on (${why}); `
    + `swallowed so far: ${s.keywords_swallowed || 0} (capture_open ${s.keywords_swallowed_capture_open || 0}, `
    + `segment_race ${s.keywords_swallowed_segment_race || 0}, repeat ${s.keywords_swallowed_repeat || 0})`);
}
const kw0 = (r) => canonicalKeyword(r && r.keyword ? r.keyword : '');
...
// 8 a minute was set when three Parakeet workers with an uncapped MLX cache
// ballooned to 8 GB each and drove the machine into 25 GB of swap. The cache
// is capped now and transcription answers in 150-900 ms, so the old ceiling
// was throttling a quarter of all segments - 125 of 393 on one camera in two
// hours - and since the keyword waits for the transcript (#29), a throttled
// segment is a lost wake rather than a slower one. Sustained demand is about
// 4 a minute; the bursts are what matter.
const SEG_RATE_MAX = intEnv('SEG_RATE_MAX', 24), SEG_PAUSE_MS = 60000, SEG_FAIL_STREAK = 3;
// POST /speech/clip, per calling address (#38). 6 a minute is invisible to a
// device that posts one clip per question and catches a loop within ten
// seconds. Per address, so a looping device is refused without silencing
// another caller.
const CLIP_RATE_MAX = intEnv('CLIP_RATE_MAX', 6);
const clipRecent = {}, clipBrakeLog = {};
// How long a segment already in flight may suppress a spotter hit (1.25.2).
// It used to suppress for the whole transcription: segPending stayed true
// from end-of-speech until the recogniser answered, and every spotter hit
// in that window was dropped without a log line or a counter. The kitchen
// is the busiest microphone in the house, so it was pending most of the
// time and "hey Oakbeard" there reached the brain once a day, by the
// transcript path, while the spotter never fired on it once. The race the
// guard was written for is real but short: the segment ends, and the
// spotter - which lags the phrase by 0.3-0.7 s - fires on its tail.
const SEG_RACE_MS = intEnv('SEG_RACE_MS', 1200);
...
function segFeed(entityId, chunk, rms) {
  ...
  segPending[entityId] = now;   // when, not just whether: the spotter guard below is time-bounded
  ...
  try {
    ...
    while (k.spotter.isReady(st.s)) {
      ...
      if (r && r.keyword) {
        ...
        const now = Date.now();
        // Every reason a heard wake phrase goes no further is counted, and
        // said out loud every 5 minutes: a silently swallowed wake is the one
        // failure the house can never explain to the person standing in it.
        if (speechCap[entityId]) { swallowedKeyword(entityId, kw0(r), 'capture_open'); continue; }
        if (segPending[entityId] && now - segPending[entityId] < SEG_RACE_MS) { swallowedKeyword(entityId, kw0(r), 'segment_race'); continue; }
        if (now - (lastKeywordMs[entityId] || 0) < 2500) { swallowedKeyword(entityId, kw0(r), 'repeat'); continue; }
        ...
        if (!KEYWORD_NEEDS_TRANSCRIPT) {
          // keyword_needs_transcript off: this is the spotter's raw guess,
          // sent before anything has agreed with it
          postHaEvent('nest_headless_keyword', {
            entity_id: entityId, camera: entityId.replace(/^camera\./, ''), keyword: kw, source: 'spotter', confirmed: false,
          }).catch(() => {});
        }
...
const handleRequest = async (req, res) => {
  ...
  // are loopback-only unless the caller presents the API token (consumer #10).
  // Snapshots, frames, detection and status stay LAN-open for HA.
  const ip = (req.socket.remoteAddress || '').replace(/^::ffff:/, '');
  const loopback = ip === '127.0.0.1' || ip === '::1';
  const sensitive = parts[0] === 'listen' || parts[0] === 'identity' || parts[0] === 'utterance' || parts[0] === 'audiodebug' || parts[0] === 'training' || parts[0] === 'speech' || (parts[0] === 'zones' && req.method !== 'GET');
  ...
  try {
    ...
    if (parts[0] === 'listen' && parts[1] && req.method === 'POST') {
      // Follow-up window (consumer #4): the brain has just spoken on this
      ...
      speechCap[entityId].openedBy = ip;   // carried on the event as opened_by so the brain can verify the source per capture (consumer #10)
    ...
    // POST /speech/clip (#38): one clip in, transcript - and optionally the
    // speaker - out, through the same engine, preprocessing and wake rule the
    // cameras use. The kitchen screen has its own spotter, so a clip only
    // arrives once the house's name has been said on the device; sending the
    // audio rather than text keeps one moment in one row, and keeps the engine
    // choice here, where it has already changed twice this week.
    // Nothing is kept and no event is emitted: this answers a question, it
    // does not sense anything.
    if (req.method === 'POST' && parts[0] === 'speech' && parts[1] === 'clip') {
      // A brake, asked for by both callers (#38). The screen is one clip per
      // question by construction, so anything faster is a bug - which makes
      // this a detector as much as a defence. It matters because this route
      // shares Parakeet with two cameras that DO have brakes, and an
      // unbraked device in a room with braked ones wins every race it should
      // lose. Refused before the audio is decoded, so a loop costs nothing,
      // and with its own reason: the person at the screen needs to hear "I'm
      // busy, ask again" rather than "I couldn't make that out".
      const nowMs = Date.now();
      const recent = (clipRecent[ip] || []).filter((t) => nowMs - t < 60000);
      if (recent.length >= CLIP_RATE_MAX) {
        clipRecent[ip] = recent;
        const retry = Math.max(1, Math.ceil((60000 - (nowMs - recent[0])) / 1000));
        if (nowMs - (clipBrakeLog[ip] || 0) > 300000) { clipBrakeLog[ip] = nowMs; console.warn(`[nest_headless] /speech/clip refused for ${ip}: over ${CLIP_RATE_MAX} clips a minute`); }
        res.writeHead(429, { 'Content-Type': 'application/json', 'Retry-After': String(retry) });
        return res.end(JSON.stringify({ ok: false, reason: 'rate_limited', limit_per_minute: CLIP_RATE_MAX, retry_after_s: retry }));
      }
      recent.push(nowMs); clipRecent[ip] = recent;
      const body = await readJsonBody(req);
      if (!body.audio_b64) { res.writeHead(400, { 'Content-Type': 'application/json' }); return res.end(JSON.stringify({ ok: false, reason: 'no_audio' })); }
      let decoded;
      try { decoded = await decodeUploadedAudio(Buffer.from(String(body.audio_b64).replace(/^data:[^,]*,/, ''), 'base64'), body.format); }
      ...
      // Transcript and speaker run on the SAME prepared samples - the caller's
      // "one clip, one moment": a name beside words they may not have said is
      // exactly what sending text separately would risk.
      const prep = prepareUploadedVoice(decoded, cfg.speechMaxSeconds || 15);
      const t = await transcribeSamples(prep.samples);
      const raw = t.text || '';
      // The clip carries its whole start (the device's spotter fired on it), so
      // the wake phrase may sit anywhere in it: the segment rule, not the
      // spotter one, which allows only a short preamble.
      // What the device says about how it was woken (#38, asked for by the iPad).
      // `wake_method`: 'spotter' - the device's own keyword spotter fired, so a
      // near-spelling in the transcript is the same evidence 1.29.0 accepts on
      // the camera path; 'button' - somebody tapped to talk, which is intent
      // stated directly and needs no phrase at all; anything else, or absent,
      // leaves the transcript to decide exactly as before.
      const wakeMethod = String(body.wake_method || '').toLowerCase();
      const wakePhrase = String(body.wake_phrase || '');
      const byTranscript = !!raw && !plainEnglishWake(raw) && (WAKE_RE.test(raw) || WAKE_ANYWHERE_RE.test(raw));
      // A clip carries its whole start, so the phrase may sit anywhere in it -
      // hence the unbounded window, matching WAKE_ANYWHERE_RE above rather than
      // the spotter's short preamble.
      const byNearSpelling = !byTranscript && wakeMethod === 'spotter' && !!raw && !plainEnglishWake(raw)
        && wakeword.spotterSpelledDifferently(wakePhrase, raw, WAKE_WORDS_STRONG, { withinChars: Infinity });
      const byDevice = ['button', 'tap', 'manual', 'press'].includes(wakeMethod);
      const confirmed = byTranscript || byNearSpelling || byDevice;
      const via = byTranscript ? 'transcript' : byNearSpelling ? 'spotter_near_spelling' : byDevice ? 'device' : null;
      const wm = WAKE_RE.exec(raw) || WAKE_ANYWHERE_RE.exec(raw);
      const num = (v) => (v === undefined || v === null || v === '' || !Number.isFinite(Number(v)) ? null : Number(v));
      const identity = String(body.identify) === 'true' ? identifyVoiceClip(prep.samples, { minScore: num(body.min_score), minGap: num(body.min_gap) }) : null;
      const out = {
        ok: true,
        text: String(body.strip_wake === undefined ? 'true' : body.strip_wake) !== 'false' ? stripWakePhrase(raw, true) : raw,
        text_raw: raw,
        // `via` says WHICH rule confirmed it, because the three are not equally
        // strong: 'transcript' heard the phrase, 'spotter_near_spelling' trusts
        // the device's spotter over the recogniser's spelling, and 'device'
        // never looked at the words at all. A consumer weighing whether to act
        // on a borderline request needs to tell those apart.
        wake: { confirmed, via, method: wakeMethod || null,
          keyword: confirmed ? (wakeword.keywordFrom(wm ? wm[0] : '', canonicalKeyword) || wakeword.keywordFrom(wakePhrase, canonicalKeyword) || 'WAKE') : null },
        engine: t.engine, stt_ms: t.sttMs,
        duration_ms: Math.round(prep.samples.length / 16),
        quality: { ...prep.quality, reason: prep.quality.speech_ms < 1000 ? 'too_short' : 'ok' },
        ...(identity ? { identity } : {}),
      };
      const who = identity && identity.matches && identity.matches[0] ? `, ${identity.matches[0].name}:${identity.matches[0].score}${identity.decisive ? ' decisive' : ''}` : '';
      console.log(`[nest_headless] CLIP "${out.text}" from ${ip} (${out.duration_ms} ms, ${t.engine} ${t.sttMs} ms, wake ${confirmed ? 'confirmed via ' + via : 'unconfirmed'}${who})`);
    ...
    if (parts[0] === 'identity') {
        ...
        if (body.image_b64) {
          ...
          try {
            ...
            if (boxes.length) {
              ...
              // No cat found. Describing the whole picture is right for a
              // close-up the detector missed - it is trained on the house
              // cameras and does poorly on phone photos - but wrong for a room
              // shot. Three pictures of an empty worktop, each 192x109, were
              // enrolled as a cat that way: camera crops posted here by
              // mistake. A real photograph is much larger.
              const meta = await sharpM(jpg).metadata();
              const short = Math.min(meta.width || 0, meta.height || 0);
              if (short < PHOTO_MIN_PX) {
                res.writeHead(400, { 'Content-Type': 'application/json' });
                return res.end(JSON.stringify({ ok: true, accepted: false, reason: 'no_cat', width: meta.width, height: meta.height,
                  hint: `no cat found and the picture is only ${meta.width}x${meta.height} - send a photo the cat is actually in` }));
              }
              found = { photo: await catid.describeImage(sharpM, jpg) };
            }
      ...
      if (req.method === 'POST' && parts[1] === 'face' && parts[2] === 'enrol') {
        const body = await readJsonBody(req);
        const cam = body.camera ? cameraEntity(body.camera) : null;
        // consumer #9: a supplied image (JSON image_b64) instead of a live frame - for a frame the
        ...
        if (body.image_b64) {
          ...
          // upload decoded raw is ~48 MB and spun the loop during onboarding, consumer #17)
          const sharpM = getSharp();
      ...
      if (req.method === 'POST' && parts[1] === 'voice' && parts[2] === 'who') {
        ...
        const num = (v) => (v === undefined || v === null || v === '' || !Number.isFinite(Number(v)) ? null : Number(v));
        const r = identifyVoiceClip(samples, { minScore: num(body.min_score), minGap: num(body.min_gap) });
        console.log(`[nest_headless] IDENTITY who from upload: ${r.matches[0] ? r.matches[0].name + ':' + r.matches[0].score : 'no match'} (${r.quality.speech_ms} ms voiced${r.decisive ? ', decisive' : ''}${r.applied && r.applied.min_score !== VOICE_DECISIVE ? ' at ' + r.applied.min_score + '/' + r.applied.min_gap : ''}) from ${ip}`);
        res.writeHead(r.ok ? 200 : 400, { 'Content-Type': 'application/json' }); return res.end(JSON.stringify(r));
      }
    ...
    if (parts[0] === 'frame' && parts[1]) {
      ...
      // the 30-45 s the consumer measured.
      const entityId = cameraEntity(parts[1]);
      const mgr = watchMgr[entityId];
...
async function handoverAsNew() {
  ...
  // asked twice: once to decide what to wait for, and again at the deadline to
  // decide whether taking over would actually be an improvement
  const outgoingLiveWatches = async () => {
    try {
      ...
      return Object.entries(st.watches || {}).filter(([, w]) => w.ready).map(([k]) => k);
    } catch (e) { return null; }   // gone or unreachable
  };
  const live0 = await outgoingLiveWatches();
  if (live0 && live0.length) wanted = wanted.filter((id) => live0.includes(id));
  ...
  for (;;) {
    ...
    if ((allUp && settled) || Date.now() - t0 > HANDOVER_MAX_WAIT_MS) {
      if (!allUp) {
        const stillLive = await outgoingLiveWatches();
        const d = handover.promotionDecision({
          readyCount: ready.length, wantedCount: wanted.length,
          outgoingLive: stillLive ? stillLive.length : 0,
          allowBlind: String(process.env.HANDOVER_ALLOW_BLIND || 'false') === 'true',
          waitedMs: Date.now() - t0,
        });
        if (!d.promote) {
          // The supervisor keeps the old instance when the standby exits, so
          // this is the whole of the fix: stand down rather than go blind.
          console.error(`[nest_headless] handover ABORTED: ${d.message}. Set HANDOVER_ALLOW_BLIND=true to override.`);
          process.exit(3);
        }
        console.warn(`[nest_headless] handover: proceeding with ${ready.length}/${wanted.length} watches live after ${Math.round((Date.now() - t0) / 1000)}s (${d.reason})`);
      }
      break;
    }
    await sleep(1000);
...
server.listen(cfg.port, () => {
  ...
  // Warming the vision models compiles ONNX graphs, which holds the event loop.
  // On a normal boot that is fine - nothing else is waiting. On a STANDBY it is
  // not: two instances share one machine during a handover, warm-up measured
  // 47 s with the loop lagging 78 s behind it, and puppeteer's 30 s wait for
  // Chrome's endpoint URL is counted on that same loop. So the browser launch
  // "timed out" while Chrome was starting perfectly well, and the standby was
  // promoted with no cameras at all (2026-09-10).
  //
  // A standby needs its cameras before it needs to recognise anything, so the
  // heavy work waits for the first watch to come up - or a cap, since a
  // deployment whose cameras never arrive must still end up warm rather than
  // stuck half-initialised.
  const warmVision = (why) => {
    infer.warmUp().then((ms) => console.log(`[nest_headless] vision models warm in ${ms} ms (${why})`)).catch((e) => console.warn('[nest_headless] warm-up failed:', e.message));
    if (faces.hasModels(FACE_MODELS_DIR())) faces.getSessions(FACE_MODELS_DIR()).catch(() => {});
  };
  if (cfg.handoverFrom) {
    const capMs = intEnv('STANDBY_WARM_CAP_S', 120) * 1000;
    const t0 = Date.now();
    const tick = setInterval(() => {
      const anyReady = Object.values(watchMgr).some((w) => w && w.ready);
      const capped = Date.now() - t0 >= capMs;
      if (!anyReady && !capped) return;
      clearInterval(tick);
      warmVision(anyReady ? 'standby, cameras up first' : `standby, cap ${Math.round(capMs / 1000)}s reached`);
    }, 1000);
    tick.unref();
  } else {
    warmVision('boot');
