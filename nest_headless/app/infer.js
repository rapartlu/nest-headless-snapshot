...
// A load that THREW is not the same as a model that is absent. The models live
// on a network share, so a failure can be "the share was busy for a moment" -
// and caching that answer disabled detection for the life of the process, with
// no error after the first one. Absence stays sticky (there is genuinely no
// model); a throw is retried, after a pause so a dead share is not hammered
// once a frame. Same shape as the browser-launch bug fixed in 1.31.1.
const LOAD_RETRY_MS = 30000;
let catSession = null, catFailedAt = 0;
async function getCatDetector() {
  if (catSession === null && fs.existsSync(CAT_PATH) && Date.now() - catFailedAt >= LOAD_RETRY_MS) {
    catSession = ort.InferenceSession.create(CAT_PATH, ORT_OPTS)
      ...
      .catch((e) => { console.warn('[nest_headless] cat detector load failed:', e.message); catSession = null; catFailedAt = Date.now(); return null; });
  }
  return catSession;
}

let detSession = null, detFailedAt = 0;
async function getDetector() {
  if (detSession === null && fs.existsSync(DET_PATH) && Date.now() - detFailedAt >= LOAD_RETRY_MS) {
    detSession = ort.InferenceSession.create(DET_PATH, ORT_OPTS)
      ...
      .catch((e) => { console.warn('[nest_headless] detector load failed:', e.message); detSession = null; detFailedAt = Date.now(); return null; });
  }
  return detSession;
}
...
async function getCls(camera) {
  ...
  let st;
  try { st = fs.statSync(p); } catch (e) { return null; }   // the share went away between the two calls
  const c = clsCache[camera];
  if (c && c.mtimeMs === st.mtimeMs && c.session) return c.session;
  // classifyDoor documents "null when no model / failure", and every caller is
  // written to that contract - but this used to throw, so a corrupt or
  // half-written model file rejected the zone tick instead. Retrained models
  // are written to a share while the add-on is running, so a half-written file
  // is a real state, not a hypothetical one.
  if (c && c.failedMtimeMs === st.mtimeMs && Date.now() - (c.failedAt || 0) < LOAD_RETRY_MS) return null;
  let session;
  try {
    session = await ort.InferenceSession.create(p, ORT_OPTS);
  } catch (e) {
    console.warn(`[nest_headless] onnx classifier ${p} failed to load: ${e.message}`);
    clsCache[camera] = { failedMtimeMs: st.mtimeMs, failedAt: Date.now() };
    return null;
  }
