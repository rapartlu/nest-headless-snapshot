...
test('a wake phrase deep in a busy segment now wakes (the kitchen case)', () => {
  const t = 'basically there are zombies in our house that are trying to eat us hey oakbeard take a photo';
...
test('a strong wake word mid-sentence does wake, which is the point', () => {
  assert.ok(wakes('no we are not eating yet hey oakbeard take a photo'));
  assert.ok(wakes('mum said no ok lantern turn the light off'));
});
