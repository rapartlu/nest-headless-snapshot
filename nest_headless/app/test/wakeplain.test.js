...
// Some spellings in wake_names are ordinary English: "heart"/"lantn" cover
// the consumer, "cloud"/"cord"/"god" cover Claude. Paired with a soft wake word
// ("a", "eh" - a quietly spoken "hey") they turn everyday speech into a wake.
const NAMES = 'oakbeard|oak beard|claude|clawed|cloud|cord|god|lantern|lantarn|lantn|hart|heart';
const SOFT = ['a', 'eh'];
const STRONG = 'hey|hi|ok|okay|hay';
const PLAIN = new Set(['heart', 'hart', 'lantn', 'cord', 'god', 'cloud', 'cloudy', 'clause', 'claws', 'clod']);
...
test('a quietly spoken "hey" still wakes before a real name', () => {
  ...
  assert.ok(!plainEnglishWake('eh oakbeard hello'));
  assert.ok(!plainEnglishWake('a lantern turn the lights off'));
});

test('a clearly spoken wake word is always honoured', () => {
  for (const t of ['ok lantern lights off', 'hey cloud what is the time', 'hi god are you there'])
