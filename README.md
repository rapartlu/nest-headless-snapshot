...
> **This repository is a published snapshot.** It is a working Home Assistant
> add-on and the code here runs a real household's cameras, but active
> development happens in a private repository. Issues are not tracked here.
> The add-on is self-contained: everything household-specific — camera names,
> zone geometry, wake words, enrolled identities — lives in configuration
> outside this tree, so nothing here is tied to one house.
